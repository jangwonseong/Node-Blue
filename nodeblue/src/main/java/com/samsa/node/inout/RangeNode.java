 ...
 * 
 * <p>
 * 입력 값이 {@code inputMin}과 {@code inputMax} 사이에 있는 경우 이를 비율에 따라 {@code outputMin}과 {@code outputMax}
 * 사이의 값으로 매핑합니다. {@code constrainToTarget}이 {@code true}인 경우, 출력 값이 출력 범위를 벗어나지 않도록 제한합니다.
 * </p>
 */
@NodeType("RangeNode")
@Slf4j
public class RangeNode extends InOutNode {
    ...
    /**
     * 지정된 입력 및 출력 범위와 함께 {@code RangeNode} 객체를 생성합니다.
     *
     ...
     */
    @JsonCreator
    public RangeNode(@JsonProperty("inputMin") double inputMin,
            @JsonProperty("inputMax") double inputMax, @JsonProperty("outputMin") double outputMin,
            @JsonProperty("outputMax") double outputMax,
            @JsonProperty("constrainToTarget") boolean constrainToTarget) {
    ...
    /**
     * 지정된 UUID와 범위를 사용하여 {@code RangeNode} 객체를 생성합니다.
     *
     ...
     *        <p>
     *        메시지의 페이로드는 {@code Number} 타입이어야 하며, 입력 범위에 따라 매핑됩니다.
     *        </p>
     */
    @Override
    protected void onMessage(Message message) {
        ...
        if (!(payload instanceof Number)) {
            log.error("잘못된 페이로드 타입. NodeId: {}, Expected: Number, Actual: {}", getId(),
        ...
        try {
            ...
            log.debug("값 매핑 완료. NodeId: {}, Input: {}, Output: {}", getId(), inputValue,
                    mappedValue);
            ...
            log.error("값 매핑 중 오류 발생. NodeId: {}, MessageId: {}", getId(), message.getId(), e);
        }
    }
