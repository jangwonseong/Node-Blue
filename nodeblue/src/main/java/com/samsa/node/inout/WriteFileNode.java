 ...
 * <p>
 * 수신한 {@link Message} 객체의 페이로드를 파일에 작성합니다.
 * 파일 추가 모드 여부를 설정할 수 있으며, 파일 쓰기 실패 시 {@link RuntimeException}을 발생시킵니다.
 * </p>
 *
...
public class WriteFileNode extends InOutNode {
     ...
     * @param append   {@code true}일 경우 기존 파일에 데이터를 추가합니다. 
     *                 {@code false}일 경우 기존 파일을 덮어씁니다.
     ...
     * <p>
     * 메시지의 페이로드가 {@code null}일 경우 처리를 중단하며 에러 로그를 출력합니다.
     * 페이로드를 문자열 형태로 변환하여 파일에 작성하고,
     * 파일 쓰기 완료 후 상위 클래스의 {@code onMessage()}를 호출하여 메시지를 다음 노드로 전달합니다.
     * </p>
     *
