...
/**
 * 입력을 받아 처리하는 노드의 추상 클래스입니다. 하나 이상의 입력 파이프를 가질 수 있으며, 입력된 메시지를 처리합니다.
 * 
 * @author samsa
 * @since 1.0
 */
...
public abstract class InNode extends Node {
    ...
    // Setter로 포트를 나중에 설정
    public void setPort(InPort port) {
        if (port == null) {
    ...
    /**
     * 입력 포트로부터 메시지를 수신합니다. 포트가 초기화되지 않았거나 메시지 수신 중 오류가 발생하면 예외가 발생합니다.
     *
